...
- Semantic highlighting from the language server: an MTA native, a Lua library
  call, a method, and one of your own functions each read differently, and a
  native carries the environment it belongs to.
- The `Luam Dark` and `Luam Light` colour themes.
...
`Luam Dark` and `Luam Light` are included. Installing the extension does not
change your colours — pick one from **File → Preferences → Theme → Color
Theme**.

Both files under `themes/` are generated by `@luam/theme` from its role table and
are never edited by hand. `pnpm --filter luam bundle` regenerates them first, and
CI fails when a committed file no longer matches the generator.
