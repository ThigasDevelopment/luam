...
import { resolveTargets } from '@cli/commands/resource-targets';
...
export async function runBuildCommand(context: CommandContext): Promise<number> {
    ...
    const result = writeResource(targets.outDir, outcome.build, options);
    ...
    reporter.info(`Wrote ${pluralize(result.written.length, 'file')} to "${targets.outDir}" (${counts}).`);
    reportPhaseTimings(reporter, tracker.durations(), totalDuration(tracker.durations()));
