...
function collectClassMember(state: CollectorState, block: BlockContext, owner: string, member: ClassMember): void {
    ...
    const detail = signatureText(member.name, member.parameters, member.returnAnnotation, inferredReturn, member.typeParameters, member.isAsync);
    ...
    collectFunctionScope(state, block, {
        ...
        body: member.body,
        selfType: member.isStatic ? null : owner,
        container: owner,
        owner: member.name,
